...
// v1 → v2 added optional `awards`, `publications` sections and an
// optional `gpa` field on education-style items. v1 payloads remain
// renderable: missing arrays normalise to [] and missing gpa to "".
...
//  * @property {string} [gpa]   Optional inline GPA / CGPA pill on education entries (e.g. "9.4/10 CGPA", "92.4%"). Renders as a small chip on the right of the label line.
//  */
//
// /**
...
//  * @property {ResumeBulletItem[]} [awards]             Awards / recognitions / honours; label = award name, body = scope and significance, meta = year + issuer.
//  * @property {ResumeBulletItem[]} [publications]       Published / accepted papers, articles, talks; label = title, body = venue + role, meta = date.
...
// Render order + display titles for the bullet-style sections.
// Single source of truth — every renderer (web ResumeTemplate +
// the three @react-pdf templates in src/resumePdf/) iterates this
// list instead of hardcoding the order or the human title. Adding
// a new section means one edit here.
export const RESUME_BULLET_SECTIONS = [
  { key: "education",          title: "Education" },
  { key: "standardized_tests", title: "Standardized tests" },
  { key: "awards",             title: "Awards & recognitions" },
  { key: "publications",       title: "Publications" },
  { key: "internships",        title: "Internships" },
  { key: "volunteer",          title: "Volunteer work" },
  { key: "activities",         title: "Co-curricular profile" },
];

// Inline-strip sections (rendered as `value · value · value` rather
// than as bullets). Same single-source-of-truth contract as
// RESUME_BULLET_SECTIONS.
export const RESUME_INLINE_SECTIONS = [
...
export function normalizeResumeJson(raw) {
  ...
  return {
    ...
    awards: arrayOfItems(j.awards),
    publications: arrayOfItems(j.publications),
