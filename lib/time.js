...
// Slice the YYYY-MM-DD prefix off a value that may be either a bare
// "YYYY-MM-DD" or a full ISO timestamp. Postgres DATE columns deserialize
// as "2026-04-29T00:00:00.000Z" via node-postgres (not "2026-04-29"), so
// any string compare against todayIstYmd() needs to slice first. Using
// this helper keeps that gotcha in one place.
//
// IMPORTANT: only call with strings (ISO or YYYY-MM-DD) or null/undefined.
// Passing a Date object would `String(date)` to "Tue Apr 29 2026 …" and
// the slice would give garbage. node-postgres returns DATE/TIMESTAMPTZ
// as strings via JSON serialization, so this isn't normally hit, but
// don't pass `new Date()` directly.
export function dateOnlyYmd(value) {
  if (!value) return "";
  return String(value).slice(0, 10);
}

// Validate that a string is BOTH a "YYYY-MM-DD" shape AND a real
// calendar date. The shape regex alone passes things like "2026-13-40"
// or "9999-99-99", which Postgres then rejects with a constraint error
// — surfacing as a 500 from the global error handler instead of a 400.
// Round-tripping the parsed Date back to its components catches
// out-of-range months/days cleanly.
export function isValidYmd(s) {
  if (typeof s !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(s)) return false;
  const [y, m, day] = s.split("-").map(Number);
  // Reject year 0 — Postgres DATE accepts year 1+. Practically
  // unrealistic input either way.
  if (y < 1) return false;
  // setUTCFullYear with an explicit year bypasses the legacy 0-99 →
  // 1900+ quirk that Date.UTC has, so "0001-01-01" round-trips
  // correctly. Date construction from "0" is just to get a valid
  // starting point.
  const d = new Date(0);
