    ...
    pages: [
      {
        ...
        fields: [
          ...
          { id: "ielts_result", label: "IELTS · result", type: "file", optional: true },
          { id: "ielts_score", label: "IELTS · overall score", type: "text", optional: true, placeholder: "7.5" },
          // TOEFL
  ...
  for (const chapter of CHAPTERS) {
    for (const page of chapter.pages) {
      ...
      if (page.requireAtLeastOne) {
        const any = page.fields.some((f) => isFilled(safe[f.id], isUploaded));
        if (!any) {
          missing.push({
            pageId: page.id,
            fieldId: null,
